...
const displayTeamMember = idx => <TeamMemberBadge key={idx} styles={styles} />;

const displayTeamMembers = teamSize => (
  <dd>
    {Array.from({ length: teamSize }, (v, i) => i).map(idx =>
      displayTeamMember(idx)
    )}
  </dd>
);
...
export const ProjectTeamSize = ({ size }) => (
  <div>
    <dt>Team</dt>
    {size < 5 ? displayTeamMembers(size) : displayTeamMembersCount(size)}
  </div>
);
